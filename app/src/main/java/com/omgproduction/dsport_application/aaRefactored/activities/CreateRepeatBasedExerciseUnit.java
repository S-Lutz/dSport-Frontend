...
import com.omgproduction.dsport_application.aaRefactored.adapter.RepeatBasedSetAdapter;
...
import com.omgproduction.dsport_application.aaRefactored.models.nodes.sets.RepeatBasedSet;
import com.omgproduction.dsport_application.aaRefactored.services.ExerciseUnitService;

import java.util.ArrayList;
import java.util.Map;

public class CreateRepeatBasedExerciseUnit extends TimeTrackingActivity implements View.OnClickListener, RepeatBasedSetAdapter.OnSetLongClickedListener {



    private RecyclerView recyclerView;
    private RepeatBasedSetAdapter adapter;
    private LinearLayoutManager layoutManager;

    private ArrayList<RepeatBasedSet> sets;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.new_activity_create_repeat_exercise_unit);
    ...
    private void findViews() {
        ...
        addSetBtn = (Button) findViewById(R.id.create_weight_exercise_unit_add_set_bt);
        addTimedBtn = (Button) findViewById(R.id.create_weight_exercise_unit_start_time_set_btn);
        finishBtn = (Button) findViewById(R.id.create_weight_exercise_unit_finish_and_save_btn);
    ...
    private void setupRecyclerView() {
        recyclerView = (RecyclerView) findViewById(R.id.create_weight_exercise_unit_recycler);
        layoutManager = new LinearLayoutManager(this);
        recyclerView.setLayoutManager(layoutManager);
        adapter = new RepeatBasedSetAdapter(sets);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.create_weight_exercise_unit_add_set_bt:
                addSet(0);
                break;
            case R.id.create_weight_exercise_unit_start_time_set_btn:
                onTimeSetStarted();
                break;
            case R.id.create_weight_exercise_unit_finish_and_save_btn:
                saveExerciseUnit();
                break;
        }
    }
    ...
    private void saveExerciseUnit() {
        if (sets.isEmpty()) {
            ...
            exerciseUnitService.addSet(this, currenExerciseUnit.getId(), sets, new BackendCallback<ExerciseUnitNode>() {
                @Override
                public void onSuccess(ExerciseUnitNode result, Map<String, String> responseHeader) {
                }
    ...
    private void onTimeSetStarted() {
        if (!started) {
            ...
            new AlertDialog.Builder(this)
                    .setTitle("Add Set")
                    .setMessage("Are you sure you want to add this set with " + currentTime / 1000 + "seconds?")
                    ...
                    .setNegativeButton(android.R.string.no, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            new AlertDialog.Builder(CreateRepeatBasedExerciseUnit.this)
    ...
    private void addSet(long time) {
        RepeatBasedSet set = new RepeatBasedSet(String.valueOf(time), "0", "0");
        sets.add(set);
        adapter.notifyItemRangeChanged(sets.size() - 1, sets.size());
    }

    @Override
    public void onLongClicked(final int position, RepeatBasedSet set) {
        new AlertDialog.Builder(this)
                .setTitle("Delete set")
