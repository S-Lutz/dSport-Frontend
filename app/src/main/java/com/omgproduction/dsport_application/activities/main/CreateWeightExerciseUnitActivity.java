...
public class CreateWeightExerciseUnitActivity extends TimeTrackingActivity implements CreateWeightSetAdapter.OnSetLongClickedListener, ExerciseUnitKeys{

    private RecyclerView recyclerView;
    private CreateWeightSetAdapter adapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_create_weight_exercise_unit);
        sets = new ArrayList<>();
        ...
        adapter = new CreateWeightSetAdapter(sets);
    ...
    private void saveExerciseUnit() {
        ...
        if(sets.isEmpty()){
            ...
            exerciseService.saveWeightExercise(localUser.getId(), Integer.parseInt(exercise.getId()), exercise.getTitle(), sets, new RequestFuture<Void>(){
                @Override
                public void onSuccess(Void result) {
                    CreateWeightExerciseUnitActivity.super.onBackPressed();
                    CreateWeightExerciseUnitActivity.super.finish();
                }

                @Override
                public void onFailure(int errorCode, String errorMessage) {
                    printError(R.id.create_weight_exercise_unit_activity,errorMessage);
                }
            });
        }
    }
        ...
        if(!started){
            ...
            new AlertDialog.Builder(this)
                    ...
                    .setNegativeButton(android.R.string.no, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            new AlertDialog.Builder(CreateWeightExerciseUnitActivity.this)
    ...
    private void addSet(long time) {
        WeightSet set = new WeightSet(time, 0, 0);
        sets.add(set);
        adapter.notifyItemRangeChanged(sets.size()-1, sets.size());
    }

    @Override
    public void onLongClicked(final int position, WeightSet set) {
        new AlertDialog.Builder(this)
                .setTitle("Delete set")
